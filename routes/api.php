...
|--------------------------------------------------------------------------
|
| These routes are protected by Sanctum bearer token authentication,
| per-token IP whitelist, per-token rate limiting, and per-resource
| ability checks.
|
*/
Route::prefix('v1')->middleware(['auth:sanctum', CheckApiWhitelistIp::class, ApiRateLimiter::class])->group(function () {
    ...
    // Services (no create — services come from orders)
    ...
    // Orders (no create/update — orders come from checkout)

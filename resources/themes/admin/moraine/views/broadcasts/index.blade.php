    ...
    <div class="flex flex-col md:flex-row gap-4 justify-between items-center">
        <div class="w-full md:w-100">
            <form action="{{ route('admin.broadcasts') }}" method="GET" class="relative inline-block max-w-150 w-full group">
                ...
                <input type="text" name="searchBroadcast" id="searchBroadcast" placeholder="{{ __('admin/common.search') }}" value="{{ request('searchBroadcast') }}" class="w-full px-6 py-3 pl-10 bg-white placeholder:text-gray-400 border-2 border-billmora-2 rounded-xl group-focus-within:outline-2 outline-billmora-primary">
        ...
        @can('broadcasts.create')
            <a href="{{ route('admin.broadcasts.create') }}" class="flex gap-1 items-center bg-billmora-primary hover:bg-billmora-primary-hover px-3 py-2 ml-auto text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                @can('broadcasts.update')
                                    <a href="{{ route('admin.broadcasts.edit', ['id' => $broadcast->id]) }}" class="inline-flex items-center text-sm font-semibold text-billmora-primary hover:text-billmora-primary-hover">
                                        {{ __('common.edit') }}
                                    </a>
                                @endcan
    ...
    @can('broadcasts.delete')
        @foreach ($broadcasts as $broadcast)
            <x-admin::modal.content
                ...
                <form action="{{ route('admin.broadcasts.destroy', ['id' => $broadcast->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')

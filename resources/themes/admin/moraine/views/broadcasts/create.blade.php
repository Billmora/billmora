...
@section('title', 'Broadcast Create')
...
<form action="{{ route('admin.broadcasts.store') }}" method="POST" class="flex flex-col gap-5">
    ...
    <div class="grid gap-4 bg-white p-8 border-2 border-billmora-2 rounded-2xl">
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <x-admin::tags name="broadcast_cc" label="{{ __('admin/broadcasts.cc_label') }}" helper="{{ __('admin/broadcasts.cc_helper') }}" :value="old('broadcast_cc')" />
            <x-admin::tags name="broadcast_bcc" label="{{ __('admin/broadcasts.bcc_label') }}" helper="{{ __('admin/broadcasts.bcc_helper') }}" :value="old('broadcast_bcc')" />
        </div>
        <x-admin::input type="datetime-local" name="broadcast_schedule" label="{{ __('admin/broadcasts.schedule_label') }}" helper="{{ __('admin/broadcasts.schedule_helper') }}" value="{{ old('broadcast_schedule') }}" />

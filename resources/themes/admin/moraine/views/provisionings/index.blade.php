...
<div class="flex flex-col gap-4">
    <div class="flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        @can('provisionings.create')
            <a href="{{ route('admin.provisionings.create') }}" class="flex gap-1 items-center bg-billmora-primary hover:bg-billmora-primary-hover px-3 py-2 ml-auto text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">
                    ...
                    <tbody class="divide-y-2 divide-billmora-2 bg-white">
                        @foreach ($provisionings as $provisioning)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">{{ $provisioning->manifest['version'] }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">{{ $provisioning->manifest['author'] }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                    @can('provisionings.update')
                                        <a href="{{ route('admin.provisionings.edit', ['provisioning' => $provisioning->id]) }}" class="inline-flex items-center text-sm font-semibold text-billmora-primary hover:text-billmora-primary-hover">
                                            {{ __('common.edit') }}
                                        </a>                               
                                    @endcan

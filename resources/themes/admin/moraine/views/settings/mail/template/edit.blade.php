...
@section('title', "Template Edit - {$template->id}")
...
<form action="{{ route('admin.settings.mail.template.update', $template->id) }}" method="POST" class="flex flex-col gap-5">
    ...
    <div class="flex gap-4 ml-auto">
        <a href="{{ route('admin.settings.mail.template') }}" class="bg-billmora-1 border-2 border-billmora-primary hover:bg-billmora-primary-hover px-3 py-2 text-billmora-primary hover:text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">{{ __('common.cancel') }}</a>

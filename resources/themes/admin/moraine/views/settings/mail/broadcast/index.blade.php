...
@section('title', 'Broadcast Settings - Mail')
    ...
    <div class="flex flex-col gap-4">
        <div class="flex flex-col md:flex-row gap-4 justify-between items-center">
            <div class="w-full md:w-100">
                <form action="{{ route('admin.settings.mail.broadcast') }}" method="GET" class="relative inline-block max-w-150 w-full group">
                    ...
                    <input type="text" name="searchBroadcastMail" id="searchBroadcastMail" placeholder="{{ __('admin/common.search') }}" value="{{ request('searchBroadcastMail') }}" class="w-full px-6 py-3 pl-10 bg-white placeholder:text-gray-400 border-2 border-billmora-2 rounded-xl group-focus-within:outline-2 outline-billmora-primary">
            ...
            @can('settings.mail.broadcast.create')
                    ...
                    <table class="min-w-full divide-y divide-billmora-2">
                        <thead class="bg-billmora-2">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">{{ __('admin/settings/mail.broadcast_table.subject') }}</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">{{ __('admin/settings/mail.broadcast_table.recipient_group') }}</th>

...
<div class="flex flex-col gap-5">
    <div class="flex flex-col gap-4">
        @can('settings.roles.create')
            <a href="{{ route('admin.settings.roles.create') }}" class="flex gap-1 items-center bg-billmora-primary-500 hover:bg-billmora-primary-600 px-3 py-2 ml-auto text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                    @can('settings.roles.update')
                                        <a href="{{ route('admin.settings.roles.edit', ['role' => $role->id]) }}" class="inline-flex items-center text-sm font-semibold text-billmora-primary-500 hover:text-billmora-primary-600">
                                            {{ __('common.edit') }}
                                        </a>
                                    @endcan

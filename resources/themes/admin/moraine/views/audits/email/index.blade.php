...
@section('title', 'History Audit - Email')

@section('body')
    <div class="flex flex-col gap-4">
        <div class="flex flex-col md:flex-row gap-4 justify-between items-center">
            <div class="w-full md:w-100">
                <form action="{{ route('admin.audits.email') }}" method="GET"
                    ...
                    <input type="text" name="searchHistoryMail" id="search" placeholder="{{ __('admin/common.search') }}"
                        value="{{ request('searchHistoryMail') }}"
            ...
            <div class="flex gap-4 ml-auto">
                ...
                @can('audit.email.history.delete')
                    <x-admin::modal.trigger modal="clearModalHistory" variant="open"
                        class="flex gap-1 items-center bg-red-500 hover:bg-red-600 px-3 py-2 ml-auto text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">
                        ...
                        <thead class="bg-billmora-2">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                                    #</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                                    {{ __('admin/audits/email.event_label') }}</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                                    {{ __('admin/audits/email.to_label') }}</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                                    {{ __('common.status') }}</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                            ...
                            @forelse ($histories as $history)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                        {{ $histories->total() - $loop->index }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                        <a href="{{ route('admin.audits.email.show', ['id' => $history->id]) }}"
                                            class="inline-flex items-center text-sm font-semibold text-billmora-primary-500 hover:text-billmora-primary-hover">{{ $history->event }}</a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                        @if ($history->user_id)
                                            <a href="{{ route('admin.users.summary', ['user' => $history->user_id]) }}"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                        {{ $history->created_at->format(Billmora::getGeneral('company_date_format')) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                        <a href="{{ route('admin.audits.email.show', ['id' => $history->id]) }}"
                                            class="inline-flex items-center text-sm font-semibold text-billmora-primary-500 hover:text-billmora-primary-hover">
                                            {{ __('common.view') }}
                                        </a>
                                        <a href="{{ route('admin.audits.email.preview', ['id' => $history->id]) }}"
                                            class="inline-flex items-center text-sm font-semibold text-yellow-400 hover:text-yellow-500"
                                            x-data="{ windowWidth: 900, windowHeight: 600 }" x-on:click.prevent="
    ...
    @can('audit.email.history.delete')
        <x-admin::modal.content modal="clearModalHistory" variant="danger" size="xl" position="centered"

    ...
    <div class="flex flex-col md:flex-row gap-4 justify-between items-center">
        <div class="w-full md:w-100">
            <form action="{{ route('admin.audits.system') }}" method="GET" class="relative inline-block max-w-150 w-full group">
                ...
                <input type="text" name="searchLogsSystem" id="searchLogsSystem" placeholder="{{ __('admin/common.search') }}" value="{{ request('searchLogsSystem') }}" class="w-full px-6 py-3 pl-10 bg-white placeholder:text-gray-400 border-2 border-billmora-2 rounded-xl group-focus-within:outline-2 outline-billmora-primary">
                    ...
                    <tbody class="divide-y-2 divide-billmora-2 bg-white">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                <a href="{{ route('admin.audits.system.show', ['system' => $log->id]) }}" class="inline-flex items-center text-sm font-semibold text-billmora-primary hover:text-billmora-primary-hover">{{ $log->event }}</a>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                <a href="{{ route('admin.audits.system.show', ['system' => $log->id]) }}" class="inline-flex items-center text-sm font-semibold text-billmora-primary hover:text-billmora-primary-hover">
                                    {{ __('common.view') }}
                                </a>
                            </td>
                        </tr>

...
<form action="{{ route('admin.invoices.transaction.create', ['invoice' => $invoice->id]) }}" method="POST" class="flex flex-col gap-5">
    ...
    <div class="flex gap-4 ml-auto">
        <a href="{{ route('admin.invoices.transaction', ['invoice' => $invoice->id]) }}" class="bg-billmora-1 border-2 border-billmora-primary-500 hover:bg-billmora-primary-600 px-3 py-2 text-billmora-primary-500 hover:text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">{{ __('common.cancel') }}</a>

    ...
    <div class="flex flex-col gap-4">
                        ...
                        <thead class="bg-billmora-2">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                                    {{ __('admin/modules.name_label') }}</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                                    {{ __('admin/modules.provider_label') }}</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                                    {{ __('admin/modules.is_active_label') }}</th>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">
                            ...
                            @forelse ($modules as $module)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                        {{ $module->is_active ? __('common.active') : __('common.active') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                        {{ $module->created_at->format(Billmora::getGeneral('company_date_format')) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                        @can('modules.update')
                                            <a href="{{ route('admin.modules.edit', ['module' => $module->id]) }}"
                                                class="inline-flex items-center text-sm font-semibold text-billmora-primary-500 hover:text-billmora-primary-hover">
                                                {{ __('common.edit') }}
                                            </a>
                                        @endcan
                                        @can('modules.delete')
                                            <x-admin::modal.trigger modal="deleteModal-{{ $module->id }}" variant="open"
        ...
        @can('modules.delete')
            @foreach ($modules as $module)
                <x-admin::modal.content modal="deleteModal-{{ $module->id }}" variant="danger" size="xl" position="centered"

...
<div class="flex flex-col gap-4">
    <div class="flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        @can('modules.create')
            <a href="{{ route('admin.modules.create') }}" class="flex gap-1 items-center bg-billmora-primary-500 hover:bg-billmora-primary-600 px-3 py-2 ml-auto text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">
                ...
                <table class="min-w-full divide-y divide-billmora-2">
                    <thead class="bg-billmora-2">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-slate-500 uppercase">{{ __('admin/modules.is_active_label') }}</th>
                    ...
                    <tbody class="divide-y-2 divide-billmora-2 bg-white">
                        @foreach ($modules as $module)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800">{{ $module->is_active ? __('common.active') : __('common.active') }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                    @can('modules.update')
                                        <a href="{{ route('admin.modules.edit', ['module' => $module->id]) }}" class="inline-flex items-center text-sm font-semibold text-billmora-primary-500 hover:text-billmora-primary-hover">
                                            {{ __('common.edit') }}
                                        </a>                               
                                    @endcan
                                    @can('modules.delete')

    ...
    <div class="flex flex-col gap-4 lg:flex-row">
          ...
          <div class="flex flex-row gap-2 flex-wrap">
            @foreach ($socials as $platform)
              @if ($url = Billmora::getGeneral('social_' . $platform))
                <x-client::link href="{{ $url }}" target="_blank" variant="secondary" icon="tabler-brand-{{ $platform }}" class="p-2!"/>
              @endif
            @endforeach
          </div>
        </div>
      ...
      <div class="flex flex-col gap-2 md:flex-row flex-1 justify-around">
        <div class="flex flex-col gap-1">
          ...
          <x-client::link href="/">Ticket</x-client::link>
        </div>
        <div class="flex flex-col gap-1">
          <h4 class="text-slate-800 font-bold">Store</h4>
          <x-client::link href="/">Minecraft Hosting</x-client::link>
        </div>
        @if (Billmora::getGeneral('term_tos') || Billmora::getGeneral('term_toc') || Billmora::getGeneral('term_privacy'))
          <div class="flex flex-col gap-1">
            ...
            <div>
              ...
              <x-client::link href="{{ Billmora::getGeneral('term_tos_url') ? Billmora::getGeneral('term_tos_url') : '/terms-of-service' }}">
                <span>{{ __('common.terms_of_service') }}</span>
              </x-client::link>
              @endif
              @if (Billmora::getGeneral('term_toc'))
              <x-client::link href="{{ Billmora::getGeneral('term_toc_url') ? Billmora::getGeneral('term_toc_url') : '/terms-of-condition' }}">
                <span>{{ __('common.terms_of_condition') }}</span>
              </x-client::link>
              @endif
              @if (Billmora::getGeneral('term_privacy'))
              <x-client::link href="{{ Billmora::getGeneral('term_privacy_url') ? Billmora::getGeneral('term_privacy_url') : '/privacy-policy' }}">

        ...
        <div class="w-full md:w-2/7 h-fit grid gap-4 md:sticky top-28 right-0 shrink-0">
            <div class="grid gap-2">
                ...
                <div class="bg-white p-4 border-2 border-billmora-2 rounded-2xl">
                    ...
                    <hr class="border-t-2 border-billmora-2 my-2">
                    <div class="grid">
                        <h4 class="text-sm text-slate-400 font-semibold">{{ __('client/tickets.ticket_department_label') }}</h4>
                        <span class="text-slate-500 font-medium">{{ ucwords($ticket->department ?? '-') }}</span>
                    </div>
                    <hr class="border-t-2 border-billmora-2 my-2">
                    <div class="grid">
                        <h4 class="text-sm text-slate-400 font-semibold">{{ __('client/tickets.ticket_service_label') }}</h4>
                ...
                <div class="flex gap-2 text-center">
                    ...
                    @if (Billmora::getTicket('ticketing_allow_client_close') && $ticket->status !== 'closed')
                        <x-client::modal.trigger modal="closeModal" class="w-full bg-red-500 hover:bg-red-600 px-3 py-2 text-white rounded-lg transition-colors ease-in-out duration-150 cursor-pointer">
                            {{ __('common.close') }}

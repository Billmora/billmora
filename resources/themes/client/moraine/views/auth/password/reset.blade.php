...
<body class="bg-white">
    <div class="flex flex-col lg:flex-row w-full min-h-dvh">
        <form action="{{ route('client.password.reset.store') }}" method="POST" class="w-full lg:w-1/2 h-auto p-8">
            @csrf
            <div class="max-w-140 h-full flex flex-col justify-between mx-auto">
                ...
                <div class="grid gap-6 my-auto">
                    ...
                    <div class="grid gap-3">
                        <x-client::input type="email" name="email" label="{{ __('common.email') }}" value="{{ $email }}" readonly required />
